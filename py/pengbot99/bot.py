...
async def configure_mwt_flip():
    ...
    """
    mwt_on_time = datetime(2026, 8, 9, 23, 55, tzinfo=timezone.utc)

    @tasks.loop(time=mwt_on_time.time())
    async def flip_mwt():
        # flip-on/off dates:
        mwt_on_time = datetime(2026, 8, 9, 23, 54, tzinfo=timezone.utc)
        mwt_off_time = datetime(2026, 8, 16, 23, 54, tzinfo=timezone.utc)
